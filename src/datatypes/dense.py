...
class DenseGraph:
    def __init__(
            ...
            masked=False,
            attribute_edge: Tensor=None,
            attribute_node: Tensor=None,
            pos: Tensor=None
        ):
        self.x = x
        ...
        self.attribute_edge=attribute_edge
    ...
    def to(self, device: str) -> DenseGraph:
        ...
        ret_graph = DenseGraph(
            ...
            attribute_edge=     None if self.attribute_edge is None else self.attribute_edge.to(device),
            attribute_node=     None if self.attribute_node is None else self.attribute_node.to(device),
    ...
    def type_as(self, x: Tensor):
        ...
        self.attribute_edge =   self.attribute_edge.type_as(x)
        self.attribute_node =   self.attribute_node.type_as(x)
        self.pos =   self.attribute_node.type_as(x)
        return self
    ...
    def _to_onehot_c(self, num_classes: int) -> DenseGraph:
        self.attribute_node = one_hot(self.attribute_node, num_classes=num_classes, dtype=torch.float)
        return self


    def to_onehot(self, num_classes_x: int, num_classes_e: int, num_classes_c: int) -> DenseGraph:

        if self.collapsed:
            self._to_onehot_nodes(num_classes_x)._to_onehot_edges(num_classes_e)._to_onehot_c(num_classes_c)
    ...
    def clone(self) -> DenseGraph:
        ret_graph = DenseGraph(
            ...
            attribute_node=     None if self.attribute_node is None else self.attribute_node.clone(),
            attribute_edge=     None if self.attribute_edge is None else self.attribute_edge.clone(),
            pos=     None if self.pos is None else self.pos.clone()
        )
    ...
    def __getitem__(self, idx) -> Any:
        ...
        graph = DenseGraph(
            ...
            attribute_node=      None if self.attribute_node is None else self.attribute_node[idx],
            attribute_edge=      None if self.attribute_edge is None else self.attribute_edge[idx],
            pos = None if self.pos is None else self.pos[idx],
    ...
    def __repr__(self):
        if self.y is not None:
            y_str = f'y={self.y.shape}, '
        else:
            y_str = ''
        if self.attribute_edge is not None:
            attribute_edge_str=f'attribute_edge={self.attribute_edge.shape}, '
        else:
            attribute_edge_str=''
        if self.attribute_node is not None:
            attribute_node_str=f'attribute_node={self.attribute_node.shape}, '
        else:
            attribute_node_str=''
        if self.pos is not None:
            pos_str=f'pos={self.pos.shape}, '
        else:
            pos_str=''
        return f'DenseGraph(x={self.x.shape}, edge_adjmat={self.edge_adjmat.shape}, {y_str}{attribute_edge_str}{attribute_node_str}{pos_str}node_mask={self.node_mask.shape}, masked={self.masked}, collapsed={self.collapsed})'
...
def to_dense_adj_bipartite(
    ...
    edge_attr : Tensor
        Tensor (E, F) containing the edge attribute
...
def dense_to_sparse(
    ...
    edge attribute. The number of returned edges is equal to the number of
...
def sparse_graph_to_dense_graph(sparse_graph: SparseGraph, handle_one_hot=False) -> DenseGraph:
    ...
    #4 bis - build a tensor for both edge and node attributes, with dimension suitable with batch

    try:
        attribute_edge: Tensor = sparse_graph.attribute_edge.reshape((batch_size, sparse_graph.attribute_edge.shape[1],sparse_graph.attribute_edge.shape[1]))
    except AttributeError:
        attribute_edge=None
        
    try:
        pos: Tensor = sparse_graph.pos.reshape((batch_size, sparse_graph.attribute_edge.shape[1], 3))
    except AttributeError:
        pos = None
        
    try:
        attribute_node: Tensor = sparse_graph.attribute_node.reshape((batch_size, max_num_nodes))
        attribute_node: Tensor = torch.nn.functional.one_hot((attribute_node+1).long(), 3).float()    
    except AttributeError:
        attribute_node=None
...
def dense_graph_to_sparse_graph(
    ...
    attribute_node = dense_graph.attribute_node[dense_graph.node_mask]
    
    try:
        pos = dense_graph.pos[dense_graph.node_mask]
    except TypeError:
        pos = None
        
    try:
        attribute_edge = dense_graph.attribute_edge[dense_graph.edge_mask]
    except TypeError:
        attribute_edge = None
    ...
    if batchify:
        cls = Batch
        kwargs = dict(_base_cls=SparseGraph)
    else:
        cls = SparseGraph
        kwargs = {}
    ...
    if batchify:
        sparse_graph = build_graphs_batch(
            graph =				sparse_graph,
            batch =				batch,

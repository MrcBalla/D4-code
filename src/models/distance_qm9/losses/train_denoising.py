...
# train denoising metrics for midi
...
import src.models.distance_qm9.labels as labels
...
class TrainLoss_distance(nn.Module):
    ...
    def forward(
        ...
        '''
        A = (torch.repeat_interleave(((x_i_0>=0.5)*(true_e==0)), repeats=4).reshape(len(((x_i_0>=0.5)*(true_e==0))),4)).float()
        B = A[:,1:]
        B[B==0]=3
        B[B==1]=0
        B[B==3]=1

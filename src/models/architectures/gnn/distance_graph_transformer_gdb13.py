...
class self_attention_mod(nn.Module):
    ...
    def forward(
        ...
        #############################  ----------- DISTANCE --------- #############################
        '''
        Q = self.q_proj_dist(Xout)                      # (bs, nq, dx)
        #print(Q)
        K = self.k_proj_dist(Xout)                      # (bs, nk, dx)
        V = self.v_proj_dist(Xout)                      # (bs, nk, dx)
        ...
        D1 = self.d_add(dist)                           # bs, nq, nk, dx
        D1 = D1.reshape((*dist.shape[:3], self.n_head, self.df))

        D2 = self.d_mul(dist)                           # bs, nq, nk, dx
        D2 = D2.reshape((*dist.shape[:3], self.n_head, self.df))

        A = D1 + (D2 + 1) * A

        eD1 = self.d_add_e(Eout)                           # bs, nq, nk, de
        eD1 = eD1.reshape((*Eout.shape[:3], self.n_head, self.df))

        eD2 = self.d_mul_e(Eout)                           # bs, nq, nk, dx
        eD2 = eD2.reshape((*Eout.shape[:3], self.n_head, self.df))

        A = eD1 + (eD2 + 1) * A                                       

        softmax_mask = e_mask2.expand(-1, n, -1, self.n_head)	# bs, nq, nk, n_head
        ...
        ye1 = self.y_e_add_d(y).unsqueeze(1).unsqueeze(1)               # (bs, 1, 1, dx)
        ye2 = self.y_e_mul_d(y).unsqueeze(1).unsqueeze(1)               # (bs, 1, 1, dx)

        # Send output to input dim
        weighted_V = weighted_V.flatten(start_dim=-2)	# (bs, nq, dx)
        
        newD = ye1 + (ye2 + 1) * weighted_V
...
class distance_GraphTransformer_gdb13(nn.Module):
    """
    ...
    def __init__(
        ...
        self.mlp_eigen  = nn.Sequential(
            nn.Linear(13, encdec_hidden_dims[DIM_DIST]),
            act_fn_in(),

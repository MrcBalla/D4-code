...
class DFBaseTransform(ABC):
    ...
    def inp(self, **kwargs) -> 'DFBaseTransform':
        for k, v in kwargs.items():
            setattr(self, k, v)
        return self

    def out(self, **kwargs) -> 'DFBaseTransform':
        for k, v in kwargs.items():
            setattr(self, k, v)
        return self
...
class DFCustomTransform(DFBaseTransform):
    ...
    def __init__(
            ...
            src_df: Optional[str|Tuple[str]|Dict[str, str]] = None,
            dst_df: Optional[str|Dict[str, str]] = None,
            src_consts: Optional[Dict] = None
        ):
        self.src_df = src_df
        self.dst_df = dst_df
        self.free_transform = free_transform
        if src_consts is None:
            self.src_consts = {}
        else:
            self.src_consts = src_consts
    ...
    def __call__(
        ...
        if self.src_df is not None:
            if isinstance(self.src_df, dict):
                kwargs = {
                    v: data[k]
                    for k, v in self.src_df.items()
                }
            elif isinstance(self.src_df, tuple):
                args = tuple(data[v] for v in self.src_df)
            else:
                args = (data[self.src_df], )

        # compute transform
        out = self.free_transform(*args, **kwargs, **self.src_consts)

        # update data
        if isinstance(self.dst_df, dict):
            for k, v in self.dst_df.items():
                data[k] = out[v]
        elif isinstance(self.dst_df, str):
            data[self.dst_df] = out
        # else do nothing if None

        return data	

    @property
    def input_df_list(self) -> List[str]:
        if isinstance(self.src_df, str):
            return [self.src_df]
        elif isinstance(self.src_df, tuple):
            return list(self.src_df)
        elif isinstance(self.src_df, dict):
            return list(self.src_df.keys())

    @property
    def output_df_list(self) -> List[str]:
        if isinstance(self.dst_df, str):
            return [self.dst_df]
        elif isinstance(self.dst_df, dict):
            return list(self.dst_df.keys())
    
    def args_repr(self) -> str:
        if hasattr(self.free_transform, '__name__'):
            return f'free_func={self.free_transform.__name__}'
        elif hasattr(self.free_transform, '__class__'):
            return f'free_func={self.free_transform.__class__.__name__}'
        else:
            return f'free_func={self.free_transform}'
...
class DFPipeline(DFCompose):
    ...
    def __init__(
            ...
            input_dfs: Optional[List[str]]=None,
            output_dfs: Optional[List[str]]=None,
    ...
    def __call__(
        ...
        # check if all input datafields are present
        if self.input_dfs is not None:
            for df in self.input_dfs:
                if df not in data:
                    raise ValueError(f'Input datafield {df} not found in data')

        # add input files to data
        if self.input_files is not None:
            data.update(self.input_files)

        # add output files to data
        ...
        # check if all output datafields are present
        if self.output_dfs is not None:
            for df in self.output_dfs:
                if df not in data:
                    raise ValueError(f'Output datafield {df} not found in data')
    ...
    def args_repr(self) -> str:
        return (
            f'input_files={self.input_dfs}\n'
            f'output_files={self.output_dfs}\n'
            f'{super().args_repr()}'
        )
...
class DFStoreToVariableDatafield(DFBaseTransform):
        r"""Store the value from a datafield to another
        datafield which name is stored in another
        datafield with name variable_df.

        data[datafield] -> data[data[variable_df]]
        """
    
        def __init__(self, datafield: str, variable_df: str):
            self.datafield = datafield
        ...
        def __call__(
            ...
            name = data[self.variable_df]
            
            data[name] = data[self.datafield]
...
class DFIterateOver(DFBaseTransform):
    ...
    def __call__(
        ...
        # if dict: iterate over key-value pairs
        if isinstance(iterable, dict):
            iterable = iterable.items()
        else:
        # if not dict: iterate over index-element pairs
            iterable = enumerate(iterable)

        # add tqdm if needed
        if self.use_tqdm:
            iterable = tqdm(iterable, total=size)
        ...
        for idx, elem in iterable:
            ...
            # update data with the output of the transform
            if self.save_output:
...
def guarded_include(if_: bool, do_: DFBaseTransform, with_: Optional[Dict]=None) -> DFBaseTransform|None:
    if if_:
        if with_:
            return do_(**with_)
        else:

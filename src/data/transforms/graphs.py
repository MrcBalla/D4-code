...
class DFGraphToNetworkx(DFBaseTransform):
    ...
    def __call__(
        ...
        # get networkx graphs
        nxgraphs: List[SparseGraph] = data[self.list_of_graphs_df]
        ...
        data[self.list_of_nxgraphs_df] = torch_graphs
    ...
    @property
    def output_df_list(self) -> List[str]:
        return [self.list_of_nxgraphs_df]
...
class DFGraphAttributePadding(DFBaseTransform):
    def __init__(
            self,
            one_hot_df: str=None, # if this is None, inplace
            graph_df: str=None,
    ...
    def __call__(
        ...
        max_nodes=data[self.max_nodes]
        
        if self.one_hot_df is not None:
            graph = graph.clone()
        
        # it there is no attribute this will pass without do anything 
        # this will be extended with a tensor of values: this version is good for one dimension changes
        try:
            if max_nodes>=graph.attribute_edge.shape[0]:
                graph.attribute_edge=torch.nn.functional.pad(graph.attribute_edge, (0,max_nodes-graph.attribute_edge.shape[0],0,max_nodes-graph.attribute_edge.shape[0]), value=0)
        except AttributeError:
            pass
        
        try:
            if max_nodes>=graph.pos.shape[0]:
                graph.pos=torch.nn.functional.pad(graph.pos, (0,0,0,max_nodes-graph.pos.shape[0]), value=0)
        except AttributeError:
            pass

        try:
            if max_nodes>=int(graph.attribute_node.shape[0]):
                n_tot=int(graph.attribute_node.shape[0])
                #graph.attribute_node = torch.stack(torch.split(graph.attribute_node,n_tot)).t()
                #graph.attribute_node = torch.nn.functional.pad(graph.attribute_node, (0,0,0, max_nodes-n_tot), value=0)
                graph.attribute_node = torch.nn.functional.pad(graph.attribute_node, (max_nodes-n_tot,0), value=0)
        except AttributeError:
            pass
    ...
    @property
    def input_df_list(self) -> List[str]:
        return [self.graph_df, self.max_nodes]
...
class DFGGetMaxStatistics(DFBaseTransform):
    def __init__(
            self,
            list_of_graphs_df: str,
            max_number_nodes: str
            #max_number_edges: str
    ):
    ...
    def __call__(
        ...
        data[self.max_number_nodes]=max_nodes

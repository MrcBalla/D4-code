...
class NoiseProcess (nn.Module):
	...
	# the formula is p(z_t|z_s) so s=(t-1)
	def get_sigma_pos_sq_ratio(self, s, t, **kwargs):
		return self.schedule.get_sigma_pos_sq_ratio(s_int=s, t_int=t, **kwargs)

	def get_x_pos_prefactor(self, s, t, **kwargs):
		return self.schedule.get_x_pos_prefactor(s_int=s,t_int=t, **kwargs)

	def get_alpha_pos_ts(self, s, t, **kwargs):
		return self.schedule.get_alpha_pos_ts(s_int=s,t_int=t, **kwargs)

	def get_alpha_pos_ts_sq(self, s, t, **kwargs):
		return self.schedule.get_alpha_pos_ts_sq(s_int=s,t_int=t,**kwargs)

	def get_params_posterior(self, t, **kwargs):
		return self.schedule.params_posterior(t, **kwargs)

	def get_sigma_bar(self, t, **kwargs):
		return self.schedule.get_sigma_bar(t_int=t, **kwargs)
